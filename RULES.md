...
- **Grid density is admin-configurable in production** — the **production admin panel must offer reduce/increment grid-item controls per breakpoint (mobile / tablet / desktop) for ALL content grids** — books, reflections (`PostGrid`), videos, homepage sections — not just the books grid. Grids must read their column counts from site settings (the `.book-grid` CSS custom-property seam `--book-grid-cols-mobile/tablet/desktop` in `styles.css` is the mechanism; equivalent per-grid settings for the other grids), never from hardcoded classes alone. Until that production setting ships, the hardcoded 1→2→3 progression is the default.
