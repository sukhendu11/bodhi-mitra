...
- **Content:** posts, pages, books (author + chapters as row columns — `author_name`/`author_bio_en/bn`, `chapters`/`chapter_pages` JSONB), videos, categories, tags, navigation, site settings, book-grid settings
...
> **Deployment note (2026-08-13, updated 2026-08-15):** `VITE_DATA_SOURCE` is a **build-time** flag — the local `.env` is gitignored, so on the dev/demo host it must be set as a **Production build env var** (`VITE_DATA_SOURCE=mock` for the demo, or `auto`/`supabase` when the fresh Supabase project is live) or the deployed site silently runs in real mode (no demo login buttons, real Supabase reads). After changing it, **Redeploy** (env changes don't rebuild automatically). **Production (2026-08-14, AD-029; build wiring 2026-08-15):** the frontend runs on **Hostinger Managed Node.js** — `npm run build` (Nitro **`node-server`** preset → `.output/`) then `npm start` (`node .output/server/index.mjs`), with `VITE_DATA_SOURCE=supabase` set at build (managed env in hPanel) — no Vercel, no VPS, no Docker.
...
- **Supabase is the unified backend**: Auth, PostgreSQL (ALL data — content + application), Storage, RLS. Content (posts, pages, books, videos, categories, tags, navigation, site settings, book-grid settings; book author/chapters live as columns on `books`) lives beside application data (profiles, purchases, orders, cart, progress, bookmarks, ratings, comments, notes/highlights, notifications, coupons, audit).
- **Admin target: Refine Core + shadcn/ui inside the TanStack app** — CRUD/data-handling patterns + component system; backed by Supabase via server functions. **Foundation built 2026-08-15 (P2)**: `@refinedev/core` + `@refinedev/supabase` + `@refinedev/react-table` installed; schema-driven resource registry (`src/lib/admin/resources.ts` — **11 resources** incl. site_settings/tags/notifications), mock-first dataProvider seam (`src/lib/admin/data-provider.ts` — real mode delegates to `@refinedev/supabase`), generic list/form/delete components (`src/components/admin/refine/`); **RBAC (`src/lib/admin/rbac.ts`)** — role→resource matrix (super_admin: all 11 full CRUD · admin: no profiles · editor: no orders/profiles/site-settings · author: posts edit only · moderator: view-only + orders · user: none), entry gate editor+, sidebar/action/form gated by role, demo editor account + login button; `/admin` renders the Refine shell in real AND mock mode (Refine is the mock-mode default since 2026-08-15; legacy MockAdminPanel reachable via `?admin=mock` preview). Browser-verified CRUD + RBAC (editor/admin/super_admin) + the 3 new resources offline. **Dashboard analytics (2026-08-15)**: mock-store stat cards (Books · Reflections · Videos · Orders · Purchases · Revenue — paid-only, BDT) via `src/lib/admin/dashboard-stats.ts` + RBAC-filtered Resources index; browser-verified (`scripts/verify-admin-dashboard-stats.mjs`). **Refine-doc alignment (2026-08-15)**: `accessControlProvider` (`src/lib/admin/access-control.ts`) wires the RBAC matrix into Refine's canonical `can({resource, action})` API — list/form actions gate via `useCan`; mock dataProvider `getList` implements `sorters` + `filters` (sortable headers, working list search); breadcrumb header + Refresh + sidebar role badge; save/delete Sonner toasts (`scripts/verify-refine-list-features.mjs`). **Dashboard analytics charts (2026-08-15)**: ECharts Content overview + Orders-by-status donut + Recent activity card via extended `AdminDashboardStats` (`src/components/admin/refine/DashboardCharts.tsx`, canvas-guarded for jsdom). **Revenue-by-day (2026-08-15)**: full-width saffron line chart of paid revenue per day (`bucketRevenueByDay`, zero-filled 14-day window) + demo commerce seed v2 (version marker, 6 paid orders / 9 purchases spread over ~2 weeks) so the chart is demoable offline. **Settings/SEO + stats (2026-08-15)**: Site Settings form is section-grouped (Branding · Hero · Theme · SEO · Social · Contact · Footer · Article · Reader · Commerce · Book Grid · Maintenance — all 12 `SiteConfig` groups editable) and now edits the full `seo` config group (`enable_sitemap` gates sitemap generation) + theme preset/custom_css + contact/article/reader/commerce copy (bilingual); posts/books/pages gained per-content `seo_title`/`seo_description` fields **wired into the public route heads** (posts/books `seoHead` prefer overrides over derived title/excerpt — `content-seo.test.ts` + `verify-content-seo.mjs`); dashboard resource cards show row counts + a needs-attention strip (pending orders / drafts). Remaining: server-side `requirePermission` enforcement + real-mode Supabase verification against the fresh instance. **SSR fix (2026-08-15)**: `@refinedev/react-table`'s extensionless `lodash/isEqual` ESM import broke dev SSR of `/admin` (lodash has no `exports` map; Node ESM rejects extensionless subpaths) — fixed via a vite alias to `src/lib/vendor/lodash-isEqual.ts` (ESM shim) + `ssr.noExternal`; production Nitro build was unaffected. Verify scripts now enforce zero console/page errors with no whitelist.
