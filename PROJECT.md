...
| **RBAC** | `src/lib/admin/rbac.ts` — role→resource permission matrix (super_admin: all 8 resources full CRUD · admin: no profiles · editor: no orders/profiles · author: posts edit only · moderator: view-only + orders · user: none). Sidebar tabs, New/Edit/Delete buttons and form saves are gated by role; server-side `requirePermission` enforcement pending real-mode mutation wiring |
