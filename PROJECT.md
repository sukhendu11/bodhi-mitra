...
| Aspect | Detail |
|--------|--------|
| **Provider** | **Refine Core + shadcn/ui inside the TanStack app — P2 (done 2026-08-15)**; Refine shell renders in real + mock mode (mock default since 2026-08-15; legacy MockAdminPanel via `?admin=mock` preview); historical: Strapi redirect shell |
| **Guard** | Frontend `/admin` checks Supabase Auth + role (RBAC via `user_roles`); entry gate = editor+ (`canEnterAdmin`) |
| **RBAC** | `src/lib/admin/rbac.ts` — role→resource permission matrix (super_admin: all 11 resources full CRUD · admin: no profiles · editor: no orders/profiles/site-settings · author: posts edit only · moderator: view-only + orders · user: none). Sidebar tabs, New/Edit/Delete buttons and form saves are gated by role; server-side `requirePermission` enforcement pending real-mode mutation wiring |
| **Resources** | 11 registered: books, posts, videos, pages, categories, navigation_items, orders, profiles, **site_settings** (single-row, dotted-key fields into the mock settings store), **tags** (derived from mock posts/books), **notifications** (admin-scope mock rows) |
...
| **Status** | ✅ Foundation built (mock-first, browser-verified CRUD + RBAC); real-mode Supabase dataProvider + server-side permission enforcement pending fresh-instance wiring |
...
- **Content tables in Supabase**: posts, pages, books (author + chapters denormalized on the row — `author_name`/`author_bio_en/bn`, `chapters`/`chapter_pages` JSONB), videos, categories, tags, navigation, site settings, book-grid settings — alongside application tables (cart, orders, purchases, progress, bookmarks, ratings, comments, notes/highlights, notifications, coupons, audit). One database, one owner (Supabase), RLS everywhere.
...
- **Implemented (2026-08-15)**: Refine Core + `@refinedev/supabase` + `@refinedev/react-table` with a schema-driven resource registry (`src/lib/admin/resources.ts` — **11 resources** incl. site_settings/tags/notifications), a mock-first dataProvider seam (`src/lib/admin/data-provider.ts` — real mode delegates to `@refinedev/supabase`), and generic list/create-edit/delete components (`src/components/admin/refine/`). `/admin` renders the Refine shell in real AND mock mode (Refine is the mock-mode default since 2026-08-15; the verified MockAdminPanel remains reachable via `?admin=mock` preview). CRUD verified in the browser (create/edit/delete/pagination) in mock mode.
...
- books (bilingual, status, price, ratings, featured; author + chapters denormalized: `author_name`, `author_bio_en/bn`, `chapters`/`chapter_pages` JSONB)
...
| **P1 — Supabase content model** | **Unified schema delta done 2026-08-15** (`supabase/migrations/20260815000001_unified_schema_delta.sql` + regenerated `manual-setup.sql`): `orders`/`order_items` (server-side order state machine, RLS), book amendments (`author_bio_en/bn`, `chapters`, `chapter_pages`), `book_grid_settings`, `covers` bucket. Existing 60 migrations already cover content + application tables with RLS. **Author/chapter modeling decision (2026-08-15):** kept **denormalized on `books`** — `author_name` + `author_bio_en/bn` columns, `chapters`/`chapter_pages` JSONB — matching the frontend consumer exactly (no separate `authors`/`chapters` tables; a relational `authors` table is documented as a future improvement for multi-author books/author pages, AD-029 §28). Content seeding decisions: mock content is the seed set (P3) | Schema + RLS designed and documented; migration SQL prepared (delta landed; `manual-setup.sql` complete at 61 sections) |
| **P2 — Custom admin** | **Refine + shadcn admin done 2026-08-15**: `@refinedev/core` + `@refinedev/supabase` + `@refinedev/react-table` installed; schema-driven resource registry (`src/lib/admin/resources.ts` — **11 resources**: books/posts/videos/pages/categories/navigation/orders/profiles + site_settings/tags/notifications, bilingual columns + form fields); mock-first dataProvider seam (`src/lib/admin/data-provider.ts`, `VITE_DATA_SOURCE`-gated, real mode delegates to `@refinedev/supabase`); generic list + create/edit dialog + delete confirm (`src/components/admin/refine/`); **RBAC (`src/lib/admin/rbac.ts`)** — role→resource permission matrix (super_admin: all 11 full CRUD · admin: content CRUD + structure edit + orders view, no profiles · editor: content CRUD + structure view/edit · author: content view + posts edit · moderator: view-only + orders view · user: none), entry gate editor+, sidebar/action/form gating by role, demo editor account (`editor@sabbe-satta.test`/`editor1234`) + login button; `/admin` renders the RBAC-aware Refine shell in real + mock mode (mock default since 2026-08-15; MockAdminPanel via `?admin=mock` preview); browser-verified CRUD + RBAC (editor/admin/super_admin) + site_settings/tags/notifications offline. **Dashboard analytics (2026-08-15)**: `DashboardTab` shows 6 mock-store stat cards (Books · Reflections · Videos · Orders · Purchases · Revenue — paid-only, BDT) via `src/lib/admin/dashboard-stats.ts` (`computeAdminDashboardStats` pure derivation + mock-first `getAdminDashboardStats` seam; real mode renders "—" until P4 Supabase aggregates) + an RBAC-filtered Resources quick-link index; `scripts/verify-admin-dashboard-stats.mjs` browser-verifies all 8 checks. **Refine-doc alignment (2026-08-15)**: `accessControlProvider` (`src/lib/admin/access-control.ts`) — Refine's canonical RBAC API (`can({resource, action})` → the same matrix, Refine action vocabulary normalized) wired into `<Refine>`; list actions + form writability gated via `useCan`; mock dataProvider `getList` now implements `sorters` + `filters` (click-to-sort headers, working list search across all row values); breadcrumb header + Refresh + sidebar role badge; bilingual Sonner toasts on save/delete. `scripts/verify-refine-list-features.mjs` browser-verifies the new list behaviors. **Dashboard analytics charts (2026-08-15, finefoods-template pattern)**: ECharts Content overview (total vs published per content type) + Orders-by-status donut + Recent activity card (latest admin notifications) via extended `AdminDashboardStats` (`ordersByStatus` + `recentActivity`); SSR/jsdom-safe canvas guard. **Revenue-by-day (2026-08-15)**: full-width saffron line/area chart (paid revenue per day, zero-filled 14-day window) via a new pure `bucketRevenueByDay` derivation; the demo commerce seed gained a version marker (v2) + 6 paid orders (9 purchases) spread over ~2 weeks so the chart is demoable offline (mirrors the reading-stats seed convention). **Settings/SEO expansion + stats depth (2026-08-15)**: Site Settings form groups fields into sections (Branding · Hero · Theme · SEO · Social · Contact · Footer · Article · Reader · Commerce · Book Grid · Maintenance — all 12 `SiteConfig` groups editable) — now edits the `seo` config group (`meta_desc_en/bn`, `og_image_url`, `google_analytics_id`, `enable_sitemap` — gates sitemap generation, `site_url`), theme `preset` + `custom_css`, hero/social/footer; per-content `seo_title`/`seo_description` added to posts/books/pages forms (schema parity) and **wired into the public route heads** — `posts.$slug`/`books.$slug` `seoHead` prefer the overrides over derived title/excerpt (`src/lib/__tests__/content-seo.test.ts` + `scripts/verify-content-seo.mjs` pin the contract; admin posts form edits the schema-true `seo_description`, books/pages the existing `meta_description_en/bn`). Dashboard: resource cards are clickable quick links with row-count badges (`resourceCounts`) + a needs-attention strip (`pendingOrders`/`draftContent`). Remaining: Supabase dataProvider verification against the fresh instance + server-side permission enforcement (requirePermission) when real-mode mutations are wired, migrate content resources | Admin CRUD works against Supabase; editors manage content without code; role permissions enforced in the admin UI + server-side |

**SSR fix (2026-08-15)**: `@refinedev/react-table`'s extensionless `lodash/isEqual` ESM import (lodash has no `exports` map) broke dev SSR of `/admin` — fixed via `vite.config.ts` alias → `src/lib/vendor/lodash-isEqual.ts` ESM shim + `ssr.noExternal`; production Nitro build was unaffected (bundles everything). Verify scripts now assert zero console/page errors with no whitelist.
...
| **D — Deployment milestone** | **Provision Hostinger Managed Node.js** (hPanel: Node 22, repo/file upload, env vars from §28 §10, domain + TLS, backups). Triggered after P4/P5/P6 when the app is sufficiently complete. Not a development prerequisite | `sabbesatta.com` serves the app from Hostinger; TLS + env configured; backups verified |
...
> **Workflow:** the user provisions the fresh Supabase instance and executes every setup step **manually** (no credentials shared with agents). This kit is the agent-prepared, user-executed checklist for P1 (Supabase content model) and P2 (custom admin). The **Hostinger section (P0 above) is a deferred Deployment milestone** — skip it during development; the agent will provide the exact Hostinger configuration steps when deployment is due. After the user completes a section, the agent verifies and wires the frontend (per the Mock Data Removal Strategy — feature by feature).
...
#### P0. Hostinger Managed Node.js — DEPLOYMENT MILESTONE (deferred — not a development prerequisite)

> **2026-08-15 (re-scope):** Hostinger provisioning is **not** required for local development. Development proceeds locally on the approved architecture; the app is kept Hostinger-compatible (node-server build preset + `npm start`, env vars documented below). **Provision this section only when the deployment milestone is reached** — the agent will tell the user exactly what to configure in Hostinger before deployment.

> **2026-08-15 (deployment model verified):** Nitro's `node-server` preset **is** TanStack Start's officially documented Node.js deployment mechanism — not an old-roadmap assumption. The official hosting guide (`docs/framework/react/guide/hosting`) calls Nitro "an agnostic layer" whose `nitro/vite` plugin "natively integrates with Vite Environments API as the underlying build tool for TanStack Start", and directs Vite builds deploying to Node to use the `node-server` preset. The `tanstackStart()` Vite plugin itself ships no production runtime (verified: no Nitro dependency in `@tanstack/start-plugin-core`; `srvx` is only dev/preview middleware). `npm run build` → `.output/server/index.mjs` (binds `NITRO_PORT ?? PORT`, default 3000; `NITRO_HOST || HOST`) → `npm start` = `node .output/server/index.mjs`. `package.json` gained `engines.node >= 22.12.0` (TanStack Start's requirement) so Hostinger hPanel auto-detects the Node runtime.
...
1. **Create the Hostinger Managed Node.js web app** — Hostinger hPanel → Web Apps / Managed Node.js → connect the repo (or deploy via Git / file upload per Hostinger's managed flow). The runtime auto-detects from `package.json` `engines.node` (`>=22.12.0` — added 2026-08-15), so Hostinger picks Node 22/24 automatically.
2. **Deploy the TanStack Start frontend** — build output deployable on the managed Node runtime; set env vars in hPanel (see §28 §10 — `VITE_DATA_SOURCE`, `SITE_URL`, Supabase keys, `PAYMENT_PROVIDER`, `RESEND_API_KEY`). Build/deploy: `npm run build` (Nitro `node-server` preset → `.output/` — TanStack Start's official Node deployment shape) then run `npm start` (`node .output/server/index.mjs`, binds `PORT` env — verified 2026-08-15) — implemented 2026-08-15 (P0 code-side).
...
2. **Apply schema** — Dashboard → **SQL Editor → New query** → paste **`supabase/manual-setup.sql`** (all 61 migrations concatenated, incl. the P1 unified-schema delta — orders/order_items, book amendments, book_grid_settings, covers bucket) → **Run**. Target buckets: `book-pdfs` (private), `covers`, `avatars`, `site-assets`, `documents`. (P1 delta: `supabase/migrations/20260815000001_unified_schema_delta.sql`, 2026-08-15.)
3. **Sanity check** — Tables list includes content tables (posts, pages, books, videos, categories, tags, navigation_items, site_settings, book_grid_settings; book author/chapters are columns on `books` — `author_name`/`author_bio_en/bn`, `chapters`/`chapter_pages` JSONB) + application tables (profiles, user_roles, purchases, orders, order_items, carts, cart_items, bookmarks, reading_progress, book_ratings, reader_bookmarks, reader_notes, comments, notifications, coupons, newsletter_subscribers, contact_messages, audit_log, search_analytics) with RLS enabled.
...
| Books catalog | Supabase `books` (author + chapters as row columns — `author_name`/`author_bio_en/bn`, `chapters`/`chapter_pages` JSONB) | Refine admin editorial | P3 |
...
1. **P0 — Local dev foundation** — Local dev on the approved stack (done: `node-server` preset + `npm start` 2026-08-15); Refine + shadcn/ui + Supabase unified schema + PipraPay research documented. Hostinger provisioning deferred to the Deployment milestone.
...
- **Supabase = unified backend** — Auth, PostgreSQL (ALL data: content + application), Storage, RLS. Content tables (posts, pages, books, videos, categories, tags, navigation, site settings, book-grid settings; book author/chapters live as columns on `books`) sit beside application tables (cart, orders, order items, purchases, progress, bookmarks, ratings, comments, notes/highlights, notifications, coupons, audit). Paid PDFs stay access-controlled via private buckets + signed URLs.
...
| **Content — Books** | `public.books` (author + chapters denormalized on the row: `author_name`, `author_bio_en/bn`, `chapters`/`chapter_pages` JSONB) | Digital books: bilingual metadata, covers, pricing, ratings, featured; author/chapter fields kept denormalized (no separate tables — matches the frontend consumer; relational `authors` table = future improvement) |
