...
const templates: {
  ...
  "contact-notification": (data) => ({
    ...
    html: emailLayout(`
      ...
      <a href="${process.env.SITE_URL || "https://sabbesatta.com"}/admin/comments" style="display:inline-block;padding:10px 20px;background-color:#d35400;color:#ffffff;border-radius:6px;text-decoration:none;font-size:14px;margin-top:16px;">View in Admin Panel</a>
    `, { preheader: `New message from ${data.name}` }),
    text: `New Contact Message\n\nFrom: ${data.name} <${data.email}>\n\nMessage:\n${data.message}\n\nView in Admin Panel: ${process.env.SITE_URL || "https://sabbesatta.com"}/admin/comments`,
  }),

  "contact-confirmation": (data) => ({
    subject: "We received your message — Sabbe Satta",
    html: emailLayout(`
  ...
  "purchase-confirmation": (data) => ({
    subject: `You now own "${escapeHtml(data.bookTitle)}" — Sabbe Satta`,
    html: emailLayout(`
      ...
      <div style="background-color:#f8f6f3;border-radius:8px;padding:16px;margin:16px 0;">
        ...
        <p style="margin:4px 0 0;color:#666;font-size:14px;">
          ${data.isFree ? "Free" : `Amount: BDT ${data.amountPaid.toFixed(2)}`}
        </p>
      </div>
      <div style="margin:24px 0;">
        <a href="${data.readerUrl}" style="display:inline-block;padding:10px 20px;background-color:#d35400;color:#ffffff;border-radius:6px;text-decoration:none;font-size:14px;">Start Reading</a>
      </div>
    `, { preheader: `Your purchase of "${data.bookTitle}" is confirmed.` }),
    text: `Purchase Confirmed!\n\nHi ${data.userName}, thank you for your purchase.\n\nBook: ${data.bookTitle}\nAmount: ${data.isFree ? "Free" : `BDT ${data.amountPaid.toFixed(2)}`}\n\nStart Reading: ${data.readerUrl}`,
  }),

  "newsletter-welcome": (data) => ({
    subject: "Welcome to Sabbe Satta newsletter",
    html: emailLayout(`
      <h2 style="margin:0 0 16px;font-size:18px;color:#1a1a1a;">Welcome!</h2>
      <p style="margin:0 0 16px;color:#666;line-height:1.6;">Thank you for subscribing to the Sabbe Satta newsletter.</p>
    ...
    `, { preheader: "Welcome to the Sabbe Satta newsletter." }),
    text: `Welcome!\n\nThank you for subscribing to the Sabbe Satta newsletter.\nYou will receive occasional emails with new reflections, book recommendations, and updates.\n\nIf you did not subscribe, you can safely ignore this email.`,
  }),

  "newsletter-unsubscribe-confirm": (data) => ({
    subject: "You've been unsubscribed — Sabbe Satta",
    html: emailLayout(`
      <h2 style="margin:0 0 16px;font-size:18px;color:#1a1a1a;">Unsubscribed</h2>
      <p style="margin:0 0 16px;color:#666;line-height:1.6;">You have been successfully unsubscribed from the Sabbe Satta newsletter.</p>
