    ...
    <header className="border-b border-border/60 bg-background/60 backdrop-blur-md sticky top-0 z-40">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
        ...
        <Link to="/" className="font-serif text-2xl tracking-tight flex items-center gap-3">
          {layout.logoUrl ? (
            <img src={layout.logoUrl} alt={layout.brandName} style={{ maxWidth: layout.logoMaxWidth, maxHeight: 56 }} className="object-contain" />
          ) : (
...
function Footer() {
  const { lang } = useLang();
  const layout = useLayout();
  const hasSocial = layout.social.facebook || layout.social.twitter || layout.social.instagram || layout.social.linkedin || layout.social.youtube;
