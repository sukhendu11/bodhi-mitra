...
import {
  LayoutDashboard, FileText, PlusCircle, BookOpen, ImageIcon, Video, Menu, Users,
  Settings, MessageSquare, FolderTree, Activity, Globe, type LucideIcon, X,
} from "lucide-react";
...
function buildNavSections(userRole: string | null | undefined, isSuperAdmin: boolean): NavSection[] {
  ...
  return [
    ...
    ...(showUsers
      ? [{
          ...
          items: [
            { to: "/admin/users", label: "Users & Roles", icon: Users as LucideIcon, exact: false },
          ],
        }]
      : []),
    {
              ...
              <div className="relative border-b border-amber-400/20 bg-gradient-to-r from-amber-50/90 to-amber-100/70 dark:from-amber-950/15 dark:to-amber-900/10 px-4 py-2">
                <div className="flex items-center justify-between gap-4 max-w-screen-2xl mx-auto">
                  <div className="flex items-center gap-3">
                    <span className="text-[0.5rem] uppercase tracking-[0.18em] font-semibold text-amber-700 dark:text-amber-400 bg-amber-200/50 dark:bg-amber-900/30 px-1.5 py-0.5 rounded">Demo</span>
                    <p className="text-[0.6rem] text-amber-700/70 dark:text-amber-300/60">Data may be reset at any time.</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <p className="text-[0.5rem] text-amber-600/50 dark:text-amber-400/40 font-mono hidden sm:block">admin@bodhimitra.test</p>
                    <button
            ...
            <div className="flex">
              ...
              <div className="flex-1 min-w-0 flex flex-col min-h-screen">
                ...
                <div className="flex flex-1">
                  {/* Center: Workspace */}
                  <main className="flex-1 min-w-0 overflow-y-auto px-4 md:px-6 py-6 md:py-8">
                    <Outlet />
                  </main>
